...
// #   error "CONFIG_LOGGING_SUBSYSTEM is incompatible with CONFIG_STDIO_PRINTF_TO_USART < 0"
// #endif
...
#		define CONFIG_KERNEL_TIME_SLICE_TICKS	(CONFIG_KERNEL_TIME_SLICE_US / CONFIG_KERNEL_SYSCLOCK_PERIOD_US)
#	else 
...
#define K_STACK_START_USABLE(stack_end, size) (K_STACK_START(stack_end, size) + CONFIG_THREAD_STACK_SENTINEL_SIZE)
...
#define K_THREAD_DEFINE(name, entry, stack_size, prio_flag, context_p, symbol) \
    __STATIC_ASSERT(0u, "K_THREAD_DEFINE is not supported when CONFIG_USE_STDLIB_HEAP_MALLOC_THREAD is enabled");
