...
/* Implement a thread/irq-safe flags notification subscription/notification system  */
...
#define K_FLAGS_INIT(initial_value) 			\
    {                                                   \
	.flags = initial_value,                         \
	.reset_value = initial_value,                   \
	._waitqueue = DLIST_INIT(flags._waitqueue)      \
    }

#define K_FLAGS_DEFINE(flags_name, initial_value) \
    struct k_flags flags_name = K_FLAGS_INIT(initial_value)
 ...
 * 		K_FLAGS_SET_ANY (required): Wait for any bit of the flags to be set
 * 		K_FLAGS_CONSUME: Consume the flags after notification
 * @param timeout Timeout to wait for if flags are not set
 * @return int positive value on success: Mask of bits that made the thread wake up
