 ...
 *
 * Key Features:
 * - **Bitmask Operations**: Flags can be set, cleared, and polled using bitmask operations, 
 *   allowing for fine-grained control over thread synchronization.
 * - **Thread Notifications**: When the specified conditions (flags) are met, threads 
 *   waiting on those flags are woken up, enabling responsive and efficient task coordination.
 * - **Options and Customization**: Flags operations can be customized with options 
 *   such as consuming the flags after use or scheduling a reschedule when a thread is woken up.
 *
 * Example Usage:
 *
...
typedef enum {
    ...
    K_FLAGS_SET_ALL = 1 << 1u,    ///< Wait for all bits of the flags to be set (not supported)
    K_FLAGS_CLR_ANY = 1 << 2u,    ///< Wait for any bit of the flags to be cleared (not supported)
    K_FLAGS_CLR_ALL = 1 << 3u,    ///< Wait for all bits of the flags to be cleared (not supported)
 ...
 */
int8_t k_flags_notify(struct k_flags *flags, k_flags_value_t notify_value, k_flags_options_t options);
