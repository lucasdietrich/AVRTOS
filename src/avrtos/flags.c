 ...
 * The swap_data is split into an options byte (upper 8 bits) and a mask byte (lower 8 bits).
 */
...
int8_t k_flags_notify(struct k_flags *flags, k_flags_value_t notify_value, k_flags_options_t options)
{
    ...
    while (notify_value != 0u) {
        ...
        struct k_thread *const thread = CONTAINER_OF(thread_handle, struct k_thread, wflags);
